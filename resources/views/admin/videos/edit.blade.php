...
<div class="card" style="max-width: 800px; margin: 0 auto; width: 100%;">
    <div class="card-body">
        <form action="{{ route('admin.videos.update', $video) }}" method="POST" enctype="multipart/form-data">
            ...
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 16px;">
                ...
                <div class="form-group">
                    ...
                    <select name="pillar" id="pillar" class="form-control @error('pillar') is-invalid @enderror" required>
                        ...
                        <option value="twk_kedinasan" {{ old('pillar', $video->pillar) === 'twk_kedinasan' ? 'selected' : '' }}>Simulasi TWK Kedinasan</option>
                    </select>
                    @error('pillar')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
            ...
            @php
                $isUploadDefault = old('video_source_type', ($video->is_direct_video || str_starts_with($video->video_url, 'storage/')) ? 'upload' : 'url');
            @endphp
            ...
            <div class="form-group" style="margin-bottom: 20px;">
                ...
                <div style="display: flex; gap: 16px; margin-bottom: 12px;">
                    <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; padding: 10px 16px; border: 1px solid var(--color-gray-300); border-radius: var(--border-radius-sm); background: var(--color-gray-100);" id="label_source_upload">
                        <input type="radio" name="video_source_type" value="upload" id="source_upload" {{ $isUploadDefault === 'upload' ? 'checked' : '' }} onchange="toggleVideoSource()">
                    ...
                    <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; padding: 10px 16px; border: 1px solid var(--color-gray-300); border-radius: var(--border-radius-sm); background: var(--color-gray-100);" id="label_source_url">
                        <input type="radio" name="video_source_type" value="url" id="source_url" {{ $isUploadDefault === 'url' ? 'checked' : '' }} onchange="toggleVideoSource()">
            ...
            <div id="container_upload" style="display: none;" class="form-group">
                <label for="video_file">Pilih File Video MP4 Baru (Opsional)</label>
                @if(str_starts_with($video->video_url, 'storage/'))
                    <div style="margin-bottom: 10px; padding: 8px 14px; background-color: rgba(16, 185, 129, 0.08); border: 1px solid rgba(16, 185, 129, 0.2); border-radius: var(--border-radius-sm); font-size: 0.85rem; color: var(--color-success); display: flex; align-items: center; gap: 8px;">
                        <i class="fi fi-rr-check-circle" style="font-size: 1rem;"></i>
                        <span>File video saat ini: <strong>{{ basename($video->video_url) }}</strong>. Biarkan kosong jika tidak ingin mengubah file video.</span>
                    </div>
                @endif
            ...
            <div id="container_url" style="display: none;" class="form-group">
                ...
                <input type="text" name="video_url" id="video_url" class="form-control @error('video_url') is-invalid @enderror" value="{{ old('video_url', str_starts_with($video->video_url, 'storage/') ? '' : $video->video_url) }}" placeholder="Contoh: https://www.youtube.com/watch?v=XaMRSuZSt0E">
                <div style="font-size: 0.8rem; color: var(--color-gray-600); margin-top: 4px;">
            ...
            <div class="form-group">
                ...
                <textarea name="content" id="content" class="form-control @error('content') is-invalid @enderror" style="height: 140px;" placeholder="Tulis catatan atau ringkasan video di sini...">{{ old('content', $video->content) }}</textarea>
                @error('content')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
